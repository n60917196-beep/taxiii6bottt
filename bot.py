...
async def toggle_online(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user.get("is_online") and db.get_balance(user["id"]) < 0:
        await update.message.reply_text(
            f"⚠️ Балансыңыз теріс ({db.get_balance(user['id'])} ₸). "
            "Online режиміне кіру үшін алдымен балансты толтырыңыз: 💳 Баланс толтыру.",
            reply_markup=driver_menu(user),
        )
        return
